...
<div class="container-fluid" id="editdiv">
    ...
    <div class="row">
        <div class="col-12">
            <form action="{{ URL('admin/page/update?lang=' . app()->getLocale()) }}" method="post" id="pageform">
                @csrf
                <div class="card customCard mt-4 mb-5" id="titlecard">
                    ...
                    <div class="card-body pt-2">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control required" id="title" placeholder="Enter Title" name="title" value="{{ $page['title'] ?? '' }}" data-empty="Enter Title" data-max="250" data-lengtherr="Maximum 250 characters allowed" />
                        </div>
                ...
                <div class="card customCard mt-4 mb-5" id="titlecard">
                    <div class="card-body pt-2">
                        <div class="form-group">
                            ...
                            <textarea class="form-control" name="meta_description" placeholder="Enter Meta Description" data-empty="Enter Meta Description">{{ $page['meta_description'] ?? '' }}</textarea>
                        </div>
                        <div class="form-group">
                            <label for="content">Meta Keywords</label>
                            <textarea class="form-control" name="meta_keywords" placeholder="Enter Meta Keywords" data-empty="Enter Meta Keywords">{{ $page['meta_keywords'] ?? '' }}</textarea>
                        </div>
                        <div class="form-group">
                            <label for="content">Analytics</label>
                            <textarea class="form-control" name="analytics" placeholder="Enter Analytics" data-empty="Enter Analytics">{{ $page['analytics'] ?? '' }}</textarea>
                        </div>
                    </div>
                </div>
                {{-- End meta inputs --}}
                ...
                <div class="card customCard transparent">
                    ...
                    <div class="card-body pt-2">
                        ...
                        <div class="">
                                ...
                                <div class="col-md-12 table-responsive">
                                    <table id="tab_logic{{ $k }}" class="table table-bordered table-hover table-sortable">
                                        ...
                                        <tbody>
                                            ...
                                            <tr id="{{ $k }}addr0" data-id="0" class="@if (!config('page.section.' . $slug . '.' . $k . '.static')) hidden novalidation @endif ">
                                                <td data-name="stitle">
                                                    @if (isset($v['blade']))
                                                    @include(
                                                    'Page::admin.section.' .
                                            ...
                                            <tr id="{{ $k }}addr{{ $sk + 1 }}" data-id="{{ $sk + 1 }}" class="">
                                                ...
                                                @if (!config('page.section.' . $slug . '.' . $k . '.static'))
                                                <td data-name="del"><a href="{{ URL('admin/page/sectiondelete/' . $sv['id']) }}" name="{{ $k }}del{{ $sk + 1 }}" class="btn btn-danger row-remove"><i class="far fa-trash-alt"></i></button></a>
                                                    <span class="moveupbtn" title="Move Up"><i class="fas fa-arrow-alt-circle-up"></i></span>
                                                    <span class="movedownbtn" title="Move Down"><i class="fas fa-arrow-alt-circle-down"></i></span>
                                                </td>
                                                @endif
                            ...
                            @if (!config('page.section.' . $slug . '.' . $k . '.static'))
                            <a id="add_row{{ $k }}" class="btn btn-primary pull-left text-white add_row" data-section="{{ $k }}">Add Row</a><input type="hidden" name="loopcount{{ $k }}" value="{{ count($pagesections[$k] ?? []) }}" id="loopcount{{ $k }}">
                            @endif
    ...
    <div class="modal-dialog modal-lg" role="document">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form method="post" action="{{ URL('admin/page/updateslug') }}">
                    @csrf
                    <div class="col-sm-4 float-left">
                        <div class="">

                            <input type="text" class="form-control" placeholder="Slug" name="slugchange" autocomplete="off" value="{{ $page['slug'] ?? '' }}" />
                            <input type="hidden" name="slug" value="{{ $slug }}" />
                        </div>
                    </div>
