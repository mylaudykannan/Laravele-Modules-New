...
if(isset($firstrow) || config('news.section.'.$slug.'.'.$k.'.static'))
{
    $sknew = '';
}
?>
...
?>
@foreach(config('news.section.'.$slug.'.'.$k.'.fields') as $fieldk => $fieldv)
...
<div class="form-group">
    <label for="content">{{$fieldv['label'] ?? ucfirst($fieldk)}}</label>
    <textarea class="form-control required @if(config('news.section.'.$slug.'.'.$k.'.static')) tinyarea @endif" id="{{$k}}{{$fieldk}}{{$sknew}}" name="{{$k}}{{$fieldk}}{{$sknew}}"
