...
<div class="container-fluid">
    ...
    <div class="row contentTopMrg">
        <div class="col-12">
            <form action="{{ URL('admin/draftnews/update?lang='.app()->getLocale()) }}" method="post" id="newsform">
                @csrf
                <div class="card customCard my-4">
                    <!-- Card header -->
                    <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
                        <div class="bg-gradient-primary shadow-primary border-radius-lg pt-4 pb-3 d-flex justify-content-between align-items-center">
                          <h5 class="text-white text-capitalize pl-3 mb-0">Edit Draft News</h5>
                          <a href="{{URL('admin/draftnews/add')}}" class="btn btn-default mb-0 mr-3 btn-sm text-white">ADD NEW DRAFT</a>
                        </div>
                    </div>

                    <div class="card-body pt-2">
                        <?php $previewurl = (config('news.url.'.$slug))?URL(config('news.url.'.$slug).'?preview=1'):URL('news/'.$slug.'?preview=1');?>
                        <a href="{{$previewurl}}" class="btn btn-sm btn-info float-right" target="_blank">Preview</a>

                        <div class="form-group">
                            @include('Gallery::admin.inc.mediainput',['name'=>'image','edit'=>true,'value'=>$news['image'] ?? '','class' => 'required','attr' => 'data-empty=Select&nbsp;Image','sizemessage'=>'600 x 400','showalt'=>true])
                        </div>
                        <div class="form-group">
                            <label for="title">Title</label>
                            <input type="text" class="form-control required" id="title" placeholder="Enter Title"
                                name="title" value="{{$news['title'] ?? ''}}" data-empty="Enter Title" data-max="250"
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control required" data-empty="Select Status" name="status" id="status">
                                ...
                                <option value="1" @if(isset($news['status']) && $news['status']=='1' ) selected
                                    @endif>Enable</option>
                                <option value="0" @if(isset($news['status']) && $news['status']=='0' ) selected
                    ...
                    <div class="card-body pt-2">
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control" id="meta_description" name="meta_description"
                                ...
                                data-lengtherr="Minimum 3 characters required">{{$news['meta_description'] ?? ''}}</textarea>
                        </div>
                        <div class="form-group">
                            ...
                            <textarea class="form-control" id="meta_keywords" name="meta_keywords"
                                ...
                                data-lengtherr="Minimum 3 characters required">{{$news['meta_keywords'] ?? ''}}</textarea>
                        </div>
                        <div class="form-group">
                            ...
                            <textarea class="form-control" id="analytics" name="analytics"
                                ...
                                data-lengtherr="Minimum 3 characters required">{{$news['analytics'] ?? ''}}</textarea>
                        </div>
                    </div>
                    ...
                    <div class="card-body pt-2">
                        ...
                        <div class="">
                                        ...
                                        <tbody>
                                            ...
                                            <tr id="{{$k}}addr{{$sk+1}}" data-id="{{$sk+1}}" class="">
                                                <td data-name="{{$k}}stitle">
                                                    @if(isset($v['blade']))
                                                    @include('News::admin.section.'.$v['blade'],['k'=>$k,'edit' => true])
                                                    @else
                                                    @include('News::admin.section.fields',['k'=>$k,'edit' => true])
                                                    @endif
                                                </td>
                                                @if(!config('news.section.'.$slug.'.'.$k.'.static'))
                                                <td data-name="del"><a
                            ...
                            <a id="add_row{{$k}}" class="btn btn-primary pull-left text-white add_row"
                                ...
                                value="{{count($newssections[$k] ?? [])}}" id="loopcount{{$k}}">
                            @endif
                ...
                <button type="button" class="btn btn-success pull-right" onclick="if(validation(event,'#newsform')==true) $('#newsform').submit();">Save Draft</button><a href="{{URL()->previous()}}" class="btn btn-success">Back</a><button type="button" class="btn btn-success pull-right" id="publishbtn">Publish</button>
            </form>
        </div>
    </div>
</div>
...
<script>
    ...
    $('body').on('click','#publishbtn',function(e){
        ...
        var result = $.ajax({
            ...
            url: root + '/admin/news/update?fromdraft=1&lang='+locale,
            type: "POST",
            datatype: "json",
